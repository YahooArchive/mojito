...
YUI({
     ...
     var suite = new Y.Test.Suite("Common: assetcsswithdefaultlocationclient");

     suite.add(new Y.Test.Case({

         "test assetcsswithdefaultlocationclient": function() {
             var that = this;
             Y.one('#assets_default_button').simulate('click');
             that.wait(function(){
                 //check if the css are load in the header
