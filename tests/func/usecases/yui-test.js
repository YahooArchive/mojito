YUI.add("test", function (b) {
        ...
        b.extend(c, b.Event.Target, {TEST_CASE_BEGIN_EVENT:"testcasebegin", TEST_CASE_COMPLETE_EVENT:"testcasecomplete", TEST_SUITE_BEGIN_EVENT:"testsuitebegin", TEST_SUITE_COMPLETE_EVENT:"testsuitecomplete", TEST_PASS_EVENT:"pass", TEST_FAIL_EVENT:"fail", TEST_IGNORE_EVENT:"ignore", COMPLETE_EVENT:"complete", BEGIN_EVENT:"begin", disableLogging:function () {
            ...
            switch (g.type) {
                ...
                case this.COMPLETE_EVENT:
                    f = b.substitute("Testing completed at " + (new Date()).toString() + ".\n" + "Passed:{passed} Failed:{failed} " + "Total:{total} ({ignored} ignored)", g.results);
                    e = "info";
                    break;
                ...
                case this.TEST_SUITE_COMPLETE_EVENT:
                    f = b.substitute('Test suite "' + g.testSuite.name + '" completed' + ".\n" + "Passed:{passed} Failed:{failed} " + "Total:{total} ({ignored} ignored)", g.results);
                    e = "info";
                    break;
                ...
                case this.TEST_CASE_COMPLETE_EVENT:
                    f = b.substitute('Test case "' + g.testCase.name + '" completed.\n' + "Passed:{passed} Failed:{failed} " + "Total:{total} ({ignored} ignored)", g.results);
                    e = "info";
                    break;
                default:
            ...
            if (i) {
                this.fire(this.TEST_FAIL_EVENT, {testCase:h, testName:k, error:l});
            } else {
                this.fire(this.TEST_PASS_EVENT, {testCase:h, testName:k});
            }
            h.tearDown();
            var g = (new Date()) - e._start;
            e.parent.results[k] = {result:i ? "fail" : "pass", message:l ? l.getMessage() : "Test passed", type:"test", name:k, duration:g};
            if (i) {
    ...
    b.Test.Reporter.prototype = {constructor:b.Test.Reporter, addField:function (c, d) {
        ...
        if (!this._form) {
            ...
            c.innerHTML = '<iframe name="yuiTestTarget"></iframe>';
            this._iframe = c.firstChild;
        ...
        this._fields.results = this.format(d);
        this._fields.useragent = navigator.userAgent;
        this._fields.timestamp = (new Date()).toLocaleString();
        b.Object.each(this._fields, function (f, g) {
    ...
    b.Mock.Value = function (e, c, d) {
        if (b.instanceOf(this, b.Mock.Value)) {
    ...
    if (typeof YUITest == "undefined" || !YUITest) {
        YUITest = {TestRunner:b.Test.Runner, ResultsFormat:b.Test.Format, CoverageFormat:b.Coverage.Format};
    }
