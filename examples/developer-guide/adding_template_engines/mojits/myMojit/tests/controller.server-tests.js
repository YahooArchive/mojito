    ...
    suite.add(new YUITest.TestCase({
        ...
        'test default_ve': function() {
            ...
            var test_data = { 
             ...
             A.areSame(test_data.template_engines.length,doneResults.template_engines.length);
             var arr_size = test_data.template_engines.length;
             for(var i=0;i<arr_size;i++){
               A.areSame(test_data.template_engines[i].name,doneResults.template_engines[i].name);
             }
         ...
         'test added_ve': function() {
            ...
            var test_data =  {
             ...
             "template_engines": [ "Mustache","EJS","Jade", "dust","underscore" ],
             "ul": { "title": 'Here are some of the other available rendering engines:' }
            };
            A.isObject(doneResults);
            A.areSame(test_data.title,doneResults.title);
            ...
            var arr_size = test_data.template_engines.length;
            for(var i=0;i<arr_size;i++){
               A.areSame(test_data.template_engines[i],doneResults.template_engines[i]);
            }
            A.areSame(test_data.ul.title,doneResults.ul.title);
          }
    }));
