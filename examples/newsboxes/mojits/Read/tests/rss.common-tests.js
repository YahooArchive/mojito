...
YUI.add('ReadModelRss-tests', function(Y, NAME) {
    ...
    suite.add(new YUITest.TestCase({
        ...
        'ohhai': function() {
            A.isFunction(rss.processResponse);
        },

        'empty response': function() {
            A.areSame(0, rss.processResponse({}).length);
        },

        'process 20 in/10 out': function() {
            A.areSame(20, mockresp().query.results.item.length);
            A.areSame(10, rss.processResponse(mockresp()).length);
        },
        ...
        'skipped 2 stories': function() {
            ...
            A.areSame(8, rss.processResponse(resp).length);
        },

        'processError exists': function() {
            A.areSame('no foo in your bar', rss.processError({query: {diagnostics: {url: {'http-status-message': 'no foo in your bar'}}}}));
        },

        'processError not exists is falsey': function() {
            A.areSame(!false, !rss.processError({query: {}}));
            A.areSame(!false, !rss.processError({}));
            A.areSame(!false, !rss.processError(''));
        },

        'processResponse strips tags': function() {
            var list = rss.processResponse(mockresp());
...
}, '0.0.1', {requires: ['mojito-test', 'ReadModelRss']});
