...
/**
 * A utility function for compiling a list of middleware by appending the
 * mojito middlewares to the ones defined by the app.
 *
 * App specific middlewares are always executed before Mojito ones.
 *
 * If user specify at least one mojito specific middleware, Mojito will use the
 * user middleware configuration "as is".
 *
 * @method _makeMiddlewareList
 * @private
 * @param {Array} app_mw Middleware list specified by the app's application.json
 ...
 */
MojitoServer.prototype._makeMiddlewareList = function (app_mw, mojito_mw) {
    var m,
        hasMojito = false,
        midName,
        middleware = [];

    if (app_mw && app_mw.length > 0) {
        ...
        if (hasMojito) {
            ...
            // we assume that they know what they are doing
            middleware = app_mw;
        } else {
            // run user middleware first
 ...
 * to use() it.
 *
 * The middleware registration API is defined by the configuration object that
 * is passed to the middleware, and is defined as below.
 *
...
MojitoServer.prototype._useMiddleware = function (app, dispatcher, midDir, midConfig, middleware) {
    ...
    for (m = 0; m < middleware.length; m += 1) {
        ...
        if (0 === midName.indexOf('mojito-handler-dispatcher')) {
            // special middleware registration here, as this is very specific
            // to mojito 
            app.use(dispatcher);
        } else {

            // TODO: package all "mojito-" middlewares in their own
            // seperate npm package
            //
            // To keep the registration process simple, Mojito will
            // register the middleware using the following (in decreasing
            // priority order):
            //
            // 1. register by `moduleName`
            // 3. register by path using Mojito middleware path as base
            // 3. register by path using the `midDir` as base
            //
            // e.g. var mid = require('mojito-contextualizer');
            //              vs
            //      var mid = require(__dirname +
            //          '/lib/app/middleware/mojito-contextualizer');
            //              vs
            //      var mid = require(midDir + midName');
            // 
            // The above order is opened for discussion.
            try {
                midFactory = require(midName);
                // console.log("======== MIDDLEWARE mojito - native " + midName);
            } catch (err) {
                try {
...
MojitoServer.prototype._configureAppInstance = function(app, options) {
    ...
    middleware = this._makeMiddlewareList(appConfig.middleware, MojitoServer.MOJITO_MIDDLEWARE);
