...
/**
 * The primary Mojito server type. Instances of MojitoServer are used to run
 * the primary Mojito system and by several of the command line tools such
 * as the build and compile commands which run temporary instances so they can
 * access Mojito functionality.
 * @param {{port: number,
 *          dir: string,
 *          context: Object,
 *          appConfig: Object,
 *          verbose: boolean}} options An object containing server options. The
 *          default port is 8666. Verbose is false by default. Dir is cwd().
 * @constructor
 */
function MojitoServer(options) {
    ...
    this._configureAppInstance(options);
 ...
 *          verbose: boolean}}
 */
MojitoServer.prototype._options = {};
 ...
 */
MojitoServer.prototype._configureAppInstance = function(options) {
    ...
    this._configureYUI(Y, store, Mojito.CORE_MOJITO_MODULES);
    ...
    Y.applyConfig({ useSync: true });
    Y.use.apply(Y, Mojito.CORE_MOJITO_MODULES);
    Y.applyConfig({ useSync: false });

    loader = new Y.mojito.Loader(appConfig);

    Y.mojito.perf.instrumentConnectApp(this._app);
    ...
    if (appConfig.middleware && appConfig.middleware.length) {
        ...
        if (hasMojito) {
            ...
            for (m = 0; m < Mojito.MOJITO_MIDDLEWARE.length; m += 1) {
                middleware.push(Mojito.MOJITO_MIDDLEWARE[m]);
            }
        }
    } else {
        middleware = Mojito.MOJITO_MIDDLEWARE;
    }
    ...
    for (m = 0; m < middleware.length; m += 1) {
        ...
        if (0 === midName.indexOf('mojito-')) {
            ...
            if (midName === 'mojito-handler-dispatcher') {
                ...
                //    context: options.context
                // };
                this._app.use(midFactory(midConfig));
            }
        } else {
    ...
    startupTime = new Date().getTime() - Mojito.MOJITO_INIT;
    ...
    http.request(options, function(res) {
        ...
        res.on('end', function() {
            ...
            if (res.statusCode !== 200) {
                return callback('Could not get web page: status code: ' +
                    res.statusCode + '\n' + buffer, url);
            } else {
 ...
 * @type {Array.<string>}
 */
Mojito.CORE_MOJITO_MODULES = ['mojito', 'mojito-route-maker'];
 ...
 * @type {Array.<string>}
 */
Mojito.CORE_YUI_MODULES = ['get', 'features', 'intl-base', 'mojito'];


/**
 * The date/time the Mojit object was initialized.
 * @type {Date}
 */
...
/**
 * Flag ensuring we only invoke YUI configuration logic once.
 * @type {Boolean}
 */
Mojito._isYUIConfigured = false;
...
Mojito.createServer = function(options) {
    ...
    if (!this._isYUIConfigured) {
        this._configureYUI();
    }

    return new MojitoServer(options);
};
