...
MojitoServer.prototype._makeMwList = function (app_mw, mojito_mw) {
    var m,
        hasMojito = false,
        midName,
        middleware = [];
    ...
    if (app_mw && app_mw.length) {
        ...
        // the above if/then is equivalent to this:
        //middleware = hasMojito ? app_mw : app_mw.concat(mojito_mw);
...
MojitoServer.prototype._useMw = function (app, dispatcher, options, midConfig, middleware) {
    var m,
        midName,
        midPath,
        midBase,
...
MojitoServer.prototype._configureAppInstance = function(app, options) {
    ...
    middleware = this._makeMwList(appConfig.middleware, MojitoServer.MOJITO_MIDDLEWARE);
    ...
    function dispatcher(req, res, next) {
        ...
        if (appConfig.debugMemory) {
            outputHandler.setMemoryDebugObjects(debugConfig);
        }
    ...
    this._useMw(app, dispatcher, options, midConfig, middleware);
