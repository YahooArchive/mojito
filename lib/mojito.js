...
MojitoServer.prototype._configureYUI = function(Y, store, load) {
    ...
    // Work around a limitiation of Y.Intl/Y.Loader where it's not loading
    // all of the datatype-date-format language bundles.
    // NOTE:  Not all of these module names are valid, but the loader tolerates
    // them anyways.
    for (lang in store.yui.langs) {
        if (lang && store.yui.langs.hasOwnProperty(lang)) {
