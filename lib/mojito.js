...
var debug = require('debug')('mojito'),
    ...
    libann = require('express-annotations'),
    libmap = require('express-map'),
    libmethods = require('methods'),
...
Mojito.prototype._configureAppInstance = function (app, store, options) {
    ...
    // Sugar example usage:
    //
    //     app.mojito.get('foo', '/foo', 'foo.index', params);
    //
    // This will expand to the following calls:
    //
    //     app.get('/foo', dispatcher.dispatch('foo.index'), params);
    //     app.map('/foo', 'foo');
    //     app.map('/foo', 'get-foo.index');
    //
    libmethods.forEach(function (method) {
        var dispatch = libdispatcher.dispatch.bind(libdispatcher),
            key = Y.mojito.util.encodeRouteName;

        app.mojito[method] = function (name, path, call) {
            app[method].apply(app,
                          [path, dispatch(call)].concat([].slice.call(arguments, 3)));
            app.map(path, name);
            app.map(path, key(method, call));
        };
    });
