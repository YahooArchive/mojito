...
var libfs = require('fs'),
    ...
    LOADER_MODULE_TEMPLATE = {
        base: 'YUI.add("loader",function(Y){' +
              '}, "", {requires:["loader-base","loader-yui3","loader-app"]});',
        full: 'YUI.add("loader",function(Y){' +
                'YUI.Env[Y.version].modules=YUI.Env[Y.version].modules||{metadata};' +
              '}, "", {requires:["loader-base","loader-app"]});'
    },

    LOADER_APP_MODULE_TEMPLATE = {
        base: 'YUI.add("loader-app", function (Y) {' +
        ...
        full: 'YUI.add("loader-app", function (Y) {' +
                'Y.applyConfig({groups:{app:{' +
                    'combine: true,' +
                    'maxURLLength: 1024,' +
                    'base: "/static/",' +
...
function staticProvider(store, globalLogger) {
    ...
    function produceMeta(type, name) {
        var token = '',
            path  = '',
            app  = appMetaData[type],
            yui  = yuiMetaData[type],
            lang = REGEX_LOCALE.exec(name);

        lang = lang && lang[1]; // eg. en-US
        ...
        app  = (lang && appMetaData[type][lang]) ?
                appMetaData[type][lang] : appMetaData[type]['*'];
        yui  = (lang && yuiMetaData[type][lang]) ?
                yuiMetaData[type][lang] : yuiMetaData[type]['*'];

        // loader-app + loader definitions
        return LOADER_APP_MODULE_TEMPLATE[type]
            .replace('{metadata}', app)
            .replace(REGEX_LANG_TOKEN, token)
            .replace(REGEX_LANG_PATH,  path) +
            LOADER_MODULE_TEMPLATE[type]
            .replace('{metadata}', yui)
            .replace(REGEX_LANG_TOKEN, token)
            .replace(REGEX_LANG_PATH,  path);
    }
    ...
    return function(req, res, next) {
        ...
        for (i = 0; i < files.length; i += 1) {
            ...
            if (module.indexOf('loader-app-base') === 0) {
                result[i] = {
                    fullpath: module,
                    content: produceMeta('base', module)
                };
            } else if (module.indexOf('loader-app-full') === 0) {
                result[i] = {
                    fullpath: module,
                    content: produceMeta('full', module)
                };
            } else if (urls[module]) {
                result[i] = {
