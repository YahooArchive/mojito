...
YUI.add('mojito-action-context', function(Y, NAME) {
    ...
    ActionContext.prototype = {
        ...
        */
            ...
            // We want to inject the assets to the client,
            // But on the client side we want to have a cache so we know what is there already
            if (mojitView && mojitView.assets) {
                //if we are on the client we check if we already have that in the page
                if (context.runtime === 'client') {
                    cache = Y.namespace('Env.Mojito.ViewAssets.cache');
                    key = instance.type + ':' + meta.view.name;
