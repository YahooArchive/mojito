...
YUI.add('mojito-action-context', function(Y, NAME) {
    ...
    function attachActionContextAddons(addons, command, adapter, ac, store) {
        ...
        var addonName,
            addon,
            dependencies = {},
            mods = command.instance.yui.sorted; // << AC - required yui modules
        ...
        for (addonName in dependencies) {
            if (dependencies.hasOwnProperty(addonName)) {
                ...
                // << RIC - quick test for create addons
                //if (addonName === "core" || ac.type === "MasterMojit") {
                // << AC - only create addons that mojit has explicity "required"
                //         assumes the convention: foo => mojito-foo-addon | ac.foo.XXX
    ...
    function ActionContext(opts) {
        ...
        var controller = opts.controller,
            actionFunction = opts.command.action;

        this.action = opts.command.action;
        this.type = opts.command.instance.type;
        this.context = opts.command.context;
        this.dispatch = opts.dispatch;

        // Needs removing and accessing from addons
        ...
        attachActionContextAddons(Y.mojito.addons.ac, opts.command, opts.adapter, this, opts.store);
        ...
        // perf = Y.mojito.perf.timeline('mojito', 'ac:init', 'set up AC object',
        //         perfID);
        ...
        // Y.mojito.perf.mark('mojito', 'action:start', 'before the action', perfID);

        // Y.log('action context created, executing action "' + actionFunction +
        //     '"', 'mojito', 'qeperf');

        // perf = Y.mojito.perf.timeline('mojito', 'action:call',
        //         'the initial syncronous part of the action', perfID);
