...
expandInstanceForEnv: function(env, instance, ctx, cb) {
        ...
        getMojitTypeDetails: function(env, ctx, mojitType, dest) {
            ...
            if (!cacheValue) {
                ...
                loadNewModules = this._loadMojit(mojitType, ctx);
                ...
                details.closestLang = Y.mojito.util.findClosestLang(ctx.lang, details.langs);
                ...
                if (this.lazyLangs) {
                    this._loadedLangs[mojitType] = this._loadedLangs[mojitType] || {};
                    loadNewModules = loadNewModules || !this._loadedLangs[mojitType][details.closestLang];
                }

                if (loadNewModules) {
                    this._loadMojitModules(mojitType, details);
                }
        ...
        _loadMojit: function (mojitType) {
            ...
            if (this._unloadedDirs[mojitType]) {
                ...
                mojitRess = this.getMojitResourceVersions(mojitType);

                // These are the resolveResourceVersions calls for the yui and url RS addons,
                // which focus just on this mojit.
                this.yui._processResources(mojitRess);
                this.url._processMojitResource(mojitType);

                // Add mojit's static details.
                this.getURLDetails(mojitRess, this._staticDetails);
        ...
        _loadMojitModules: function (mojitType, details) {
            var modules = {};
            ...
            this._appY.use.apply(this._appY,  Object.keys(modules));
waterfall.end('use');
        ...
        resolveVersion: function (type, env, posl) {
            var s,
                ...
                mojitRes,
                result,
                details = this._mojitDetailsCache[type + poslString + env];
        ...
        _preloadDirMojit: function(dir, dirType, pkg) {
            ...
            if (this.lazyMojits && !this.preloaded && mojitType !== 'PipelineFrame') {
        ...
        _findResourcesByConvention: function(dir, dirType, pkg, mojitType) {
            ...
            // TODO is it possible to populate ress in a sorted manner?
