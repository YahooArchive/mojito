...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        initializer: function(cfg) {
            ...
            this._mojitDetails = {};  // mojitType: env-POSL: details
        ...
        getMojitTypeDetails: function(env, ctx, mojitType, dest, DANGERDANGERreturnRawCacheValue) {
            ...
            if (!cacheValue) {
                details = Y.mojito.util.copy(this._mojitDetails[mojitType][env + ':' + poslKey]);
        ...
        resolveMojitDetails: function(env, posl, type, ress, mojitRes) {
            ...
            for (r = 0; r < ress.length; r += 1) {
                ...
                if (res.type === 'addon') {
                    if (res.subtype === 'view-engines') {
                        // no need to track view engines
                        continue;
                    }
            ...
            if (env !== 'client') {
                clientDetails = this._mojitDetails[type]['client:' + poslKey];
                ...
                for (p = 0; p < posls.length; p += 1) {
                    ...
                    for (type in this._mojitRVs) {
                        if (this._mojitRVs.hasOwnProperty(type)) {
                            ...
                            this._mojitDetails[type][env + ':' + poslKey] = this.resolveMojitDetails(env, posl, type, ress, mojitRes);
                        }
                    }
                }
            }
