...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        initializer: function(cfg) {
            ...
            this._resCachePath = this._libs.path.join(this._config.root, 'rescache.json');
         ...
         */
        preload: function() {
            var useCache = !this._config.skipCache;
            if (useCache) {
                try {
                    this.resourceCacheLoad();
                    Y.log('Store preloaded from cache', 'info', NAME);
                } catch (err) {
                    // Ignore details of failures, but be sure to do a real load.
                    useCache = false;
                }
            }
            if (!useCache) {
                ...
                Y.log('Store preloaded from filesystem', 'info', NAME);
            }
        ...
        /**
         * Saves the current state of the store.  The details are saved into a location
         * that the store can find it later.
         * @method resourceCacheSave
         */
        resourceCacheSave: function() {
            var cache = { rs: {} };

            cache.rs._appRVs        = this._appRVs;
            cache.rs._mojitRVs      = this._mojitRVs;
            cache.rs._appResources  = this._appResources;
            cache.rs._mojitResources = this._mojitResources;
            cache.rs._appPkg        = this._appPkg;
            cache.rs.selectors      = this.selectors;

            Y.Object.each(Y.mojito.addons.rs, function(fn, name) {
                if ('function' === typeof this[fn.NS].resourceCacheSave) {
                    cache[fn.NS] = {};
                    this[fn.NS].resourceCacheSave(cache[fn.NS]);
                }
            }, this);

            this._libs.fs.writeFileSync(this._resCachePath, JSON.stringify(cache, null, 4), 'utf8');
        },


        /**
         * This method is mainly intended to be used internally.
         * Loads this store from the cache.
         * @method resourceCacheLoad
         * @return {nothing} Nothing is returned.
         */
        resourceCacheLoad: function() {
            var cache;
            cache = this._libs.fs.readFileSync(this._resCachePath, 'utf8');
            cache = JSON.parse(cache);

            this._appRVs        = cache.rs._appRVs;
            this._mojitRVs      = cache.rs._mojitRVs;
            this._appResources  = cache.rs._appResources;
            this._mojitResources = cache.rs._mojitResources;
            this._appPkg        = cache.rs._appPkg;
            this.selectors      = cache.rs.selectors;

            this.loadAddons(cache);
            Y.Object.each(Y.mojito.addons.rs, function(fn, name) {
                if ('function' === typeof this[fn.NS].resourceCacheLoad) {
                    this[fn.NS].resourceCacheLoad(cache[fn.NS]);
                }
            }, this);
         ...
         *
         * @method loadAddons
         * @param {object} cache Object containing the build-time cache for the addons.
         * @return {nothing}
         */
        loadAddons: function(cache) {
            var modules = {},
                ress,
                r,
                res;
            ...
            Y.Object.each(Y.mojito.addons.rs, function(fn, name) {
                ...
                if (cache && 'function' === typeof fn.resourceCacheLoad) {
                    fn.resourceCacheLoad(cache[fn.NS]);
                }
            }, this);
        },
        ...
        /**
         * Called by the ResourceStore to allow RS addons to create resourece
