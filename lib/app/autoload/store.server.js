...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        initializer: function(cfg) {
            ...
            this._mojitDetailRVs = {}; // MojitType: selector: environment: non-stringified details
            this._mojitDetailRVsCache = {};
        ...
        getAllURLDetails: function() {
            ...
            for (m = 0; m < mojits.length; m += 1) {
                mojit = mojits[m];
                ress = this.getResourceVersions({mojit: mojit, chosen: true});
                for (r = 0; r < ress.length; r += 1) {
                    res = ress[r];
                    if (res.url && res.source.fs.isFile) {
                        if (urls[res.url]) {
        ...
        resolveResourceVersions: function () {
            ...
            for (crtType in this._mojitRVs) {
                if (this._mojitRVs.hasOwnProperty(crtType)) {
                    ...
                    this._mojitDetailRVs[crtType] = {};

                    for (r = 0; r < resources.length; r++) {
                        crtRes = resources[r];

                        // create the new selector->affinity map if it doesn't exist
                        if (!this._mojitDetailRVs[crtType][crtRes.selector]) {
                            this._mojitDetailRVs[crtType][crtRes.selector] = {};
                        }
                        if (!this._mojitDetailRVs[crtType][crtRes.selector][crtRes.affinity]) {
                            this._mojitDetailRVs[crtType][crtRes.selector][crtRes.affinity] = [];
                        }

                        this._mojitDetailRVs[crtType][crtRes.selector][crtRes.affinity].push(crtRes);
                    }
                }
            }

            // if we don't want to lazy resolve, resolve for all posls
            if (!this._appConfigStatic.resourceStore || !this._appConfigStatic.resourceStore.lazyResolve) {
        ...
        resolveVersion: function (type, env, posl) {
            ...
            if (this._mojitDetailRVsCache[type + poslString + env]) {
                return JSON.parse(this._mojitDetailRVsCache[type + poslString + env]);
            }
            ...
            for (s = posl.length - 1; s >= 0; s--) { // ~3
                ...
                for (t = types.length - 1; t >= 0; t--) { // 2
                    ...
                    for (e = environments.length - 1; e >= 0; e--) { // 2
                        currentResources = (this._mojitDetailRVs[currentType][currentSelector] &&
                            this._mojitDetailRVs[currentType][currentSelector][environments[e]]) || [];

                        console.log('[store.server.js:1607] currentResources:' + currentSelector + ', ' + currentType + ', ' + environments[e]);
                        ...
                        for (r = 0; r < currentResources.length; r++) { // ~3
                            ...
                            if (!resolvedResources[currentResources[r].id] ||
                                    resolvedResources[currentResources[r].id].source.pkg.depth >= currentResources[r].source.pkg.depth) {
                                resolvedResources[currentResources[r].id] = currentResources[r];
                                resolvedResources[currentResources[r].id].chosen = true;
                            }
                        }
                    }
                }
            ...
            this._mojitDetailRVsCache[type + poslString + env] = JSON.stringify(result);
            return result;
