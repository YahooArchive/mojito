...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    libs.ContextCache = require('context-cache');
    libs.fs = require('fs');
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        validateContext: function(ctx) {
            ...
            cacheValue = this.contextCacheGet(ctx, 'validateContext');
            if (cacheValue) {
                if (cacheValue === 'VALID') {
                    return;
                }
            ...
            for (k in ctx) {
                if (ctx.hasOwnProperty(k)) {
                    ...
                    if (!this._validDims[k]) {
                        cacheValue = 'INVALID dimension key "' + k + '"';
                        this.contextCacheSet(ctx, 'validateContext', cacheValue);
                        throw new Error(cacheValue);
                    }
                    ...
                    if ('lang' === k) {
                        ...
                        if (!found) {
                            cacheValue = 'INVALID dimension value "' + ctx[k] + '" for key "' + k + '"';
                            this.contextCacheSet(ctx, 'validateContext', cacheValue);
                            throw new Error(cacheValue);
                        }
                        continue;
            ...
            this.contextCacheSet(ctx, 'validateContext', 'VALID');
            return true;
        },
        ...
        getAppConfig: function(ctx) {
            ...
            appConfig = this.contextCacheGet(ctx, 'appConfig');
            if (appConfig) {
                return appConfig;
            }

            ycb = this._appConfigYCB.read(this.blendStaticContext(ctx));
            ...
            this.contextCacheSet(ctx, 'appConfig', appConfig);
        ...
        /**
         * optimize for the specific runtime, or for "production" if none given in static context
         */
        ...
        resolveMojitDetails: function(env, posl, type, ress, mojitRes) {
            ...
            for (r = 0; r < ress.length; r += 1) {
                ...
                if (res.type === 'spec') {
                    ...
                    this._specPaths[res.mojit + ':' + res.name] = res.source.fs.fullPath;
                    continue;
                }
        ...
        getRoutes: function(ctx) {
            ...
            cachedValue = this.contextCacheGet(ctx, 'routes');
            if (cachedValue) {
                return cachedValue;
            }
            ...
            this.contextCacheSet(ctx, 'routes', out);
            return out;
        },
         ...
         * @method contextCacheGet
         * @param {object} ctx runtime context
         * @param {string} key the key name for the cached value
         * @return {any} the cache value, or undefined if not cached
         */
        contextCacheGet: function(ctx, key) {
            ...
            data = this._contextCache.get(ctx) || {};
            return data[key];
        },


        /**
         * Sets a value into the context cache.
         * @method contextCacheSet
         * @param {object} ctx runtime context
         * @param {string} key the key name for the cached value
        ...
        contextCacheSet: function(ctx, key, value) {
            ...
            data = this._contextCache.get(ctx) || {};
            data[key] = value;
            return this._contextCache.set(ctx, data);
        },
                ...
                for (p = 0; p < posls.length; p += 1) {
                    ...
                    for (type in this._mojitRVs) {
                        if (this._mojitRVs.hasOwnProperty(type)) {
                            ...
                            this._mojitDetails[type][env + ':' + poslKey] = JSON.stringify(
        ...
        /**
         * Creates the context cache.
         * @private
         * @method _contextCacheCreate
         * @return {nothing}
         */
        _contextCacheCreate: function() {
            var config = this._appConfigStatic.contextCache || {};
            // Mojito needs -some- kind of isolation, otherwise we definitely do bleed
            // between contexts.
            if (!config.isolationMode) {
                config.isolationMode = 'json';
            }
            this._contextCache = this._libs.ContextCache.create(config);
        },
