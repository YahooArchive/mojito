    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        resolveMojitDetails: function(env, posl, type, ress, mojitRes) {
            ...
            if (env !== 'client') {
                ...
                clientDetails = this.resolveMojitFaster(type, 'client', posl);
        ...
        resolveMojitFaster: function (type, env, posl) {
            var s,
                ...
                types = [type, 'shared'],
                e,
                environments = [env, 'common'],
                poslString = posl.toString(),
                resolvedResources = {},
                mojitRes = this.getResourceVersions({type: 'mojit', name: type})[0],
                result;

            if (this._smarterDetailsCache[type + poslString + env]) {
                return JSON.parse(this._smarterDetailsCache[type + poslString + env]);
            }
            ...
            for (s = posl.length - 1; s >= 0; s--) { // ~3
                currentSelector = posl[s];

                for (t = types.length - 1; t >= 0; t--) { // 2
                    currentType = types[t];

                    for (e = environments.length - 1; e >= 0; e--) { // 2
            ...
            this._smarterDetailsCache[type + poslString + env] = JSON.stringify(result);
            return result;
