...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        initializer: function(cfg) {
            ...
            this._mojitDetails = {};  // mojitType: env-POSL: JSONified details
        ...
        preload: function() {
            ...
            this.resolveResourceVersionsFaster();
        ...
        getMojitTypeDetails: function(env, ctx, mojitType, dest, DANGERDANGERreturnRawCacheValue) {
            ...
            if (!cacheValue) {
                ...
                details = this.resolveMojitFaster(mojitType, env, posl);

                // details = JSON.parse(this._resolveMojit(mojitType, env, posl));
                // details = JSON.parse(this._mojitDetails[mojitType][env + ':' + poslKey]);
        ...
        resolveMojitDetails: function(env, posl, type, ress, mojitRes) {
            ...
            if (env !== 'client') {
                // clientDetails = JSON.parse(this._mojitDetails[type]['client:' + poslKey]);
                clientDetails = JSON.parse(this._resolveMojit(type, 'client', posl));
                    ...
                    for (p = 0; p < posls.length; p += 1) {
                        ...
                        for (type in this._mojitRVs) {
                            if (this._mojitRVs.hasOwnProperty(type)) {
                                this._resolveMojit(type, env, posl);
                            }
                        }
                    }
                }
         ...
         * that will make it easy to retrieve it later at runtime.
         * @return {[type]} [description]
         */
        resolveResourceVersionsFaster: function () {
            var crtType,
                resources,
                r,
                crtRes;

            // for each resource of each mojit
            for (crtType in this._mojitRVs) {
                if (this._mojitRVs.hasOwnProperty(crtType)) {
                    resources = this._mojitRVs[crtType];

                    this._smarterDetails[crtType] = {};
                    ...
                    for (r in resources) {
                        if (resources.hasOwnProperty(r)) {
                            ...
                            if (!this._smarterDetails[crtType][crtRes.selector]) {
                                this._smarterDetails[crtType][crtRes.selector] = {};
                            }
                            if (!this._smarterDetails[crtType][crtRes.selector][crtRes.affinity]) {
                                this._smarterDetails[crtType][crtRes.selector][crtRes.affinity] = [];
                            }

                            this._smarterDetails[crtType][crtRes.selector][crtRes.affinity].push(crtRes);
                        }
                    }
                }
            }
        ...
        /**
         * Find a mojit details in the resource store at non-runtime-sensitive
         ...
         */
        resolveMojitFaster: function (type, env, posl) {
            var s,
                currentSelector,
                r,
                currentResources,
                ...
                resolvedResources = {},
                mojitRes = this.getResourceVersions({type: 'mojit', name: type})[0],
                result;

            if (this._smarterDetailsCache[type] &&
                    this._smarterDetailsCache[type][poslString] &&
                    this._smarterDetailsCache[type][poslString][env]) {
                return this._smarterDetailsCache[type][poslString][env];
            }

            // we go in reverse order for the posl, types and environments so that
            // lower priority resources are overwritten by the higher ones
            // with the same id
            posl.reverse();

            for (s in posl) { // ~3
                if (posl.hasOwnProperty(s)) {
                    ...
                    for (t in types) { // 2
                        if (types.hasOwnProperty(t)) {
                            ...
                            for (e in environments) { // 2
                                if (environments.hasOwnProperty(e)) {
                                    currentResources = this._smarterDetails[currentType][currentSelector] &&
                                        this._smarterDetails[currentType][currentSelector][environments[e]];

                                    // remember each resource we find for this posl, type and environment
                                    for (r in currentResources) { // ~3
                                        if (currentResources.hasOwnProperty(r)) {
                                            resolvedResources[currentResources[r].id] = currentResources[r];
                                        }
                                    }
                                }
                            }
            ...
            // prepare the cache if it doesn't exist already
            if (!this._smarterDetailsCache[type]) {
                this._smarterDetailsCache[type] = {};
            }
            if (!this._smarterDetailsCache[type][poslString]) {
                this._smarterDetailsCache[type][poslString] = {};
            }
            result = this.resolveMojitDetails(env, posl, type, Y.Object.values(resolvedResources), mojitRes);

            this._smarterDetailsCache[type][poslString][env] = result;
            return result;
        ...
        _resolveMojit: function (type, env, posl) {
            var affinities = {},
                selectors = {},
            ...
            if (!this._mojitDetails[type]) {
                this._mojitDetails[type] = {};
            }
            // While it might seem crazy to keep this in JSON format, we do it for two reasons:
            //  * It keeps the long-lived datastructure out of repeated GC runs.
            //  * When used, we need to make a copy anyways, and the data isn't too large.
            cachedMojitDetails = this._mojitDetails[type][env + ':' + poslKey];
            if (!cachedMojitDetails) {
                cachedMojitDetails = this._mojitDetails[type][env + ':' + poslKey] = JSON.stringify(
                    this.resolveMojitDetails(env, posl, type, ress, mojitRes)
                );
            }
            return cachedMojitDetails;
        },
        ...
        _resolveVersions: function(affinities, selectors, sourceBase, srcs) {
            ...
            for (resid in versions) {
                if (versions.hasOwnProperty(resid)) {
                    ...
                    versions[resid][highest].chosen = true;
