...
YUI.add('mojito-perf', function (Y, NAME) {
    ...
    var libfs = require('fs'),
        ...
        data   = YUI.namespace('Env.mojito').DataProcess,
    ...
    function mark(group, label, msg, id) {
        ...
        if (id) {
            ...
            id = Y.Lang.isObject(id) ? _idFromCommand(id) : id;
            key += '[' + id + ']';
        }
    ...
    if (config) {
        Y.mojito.perf._idFromCommand = _idFromCommand;
        Y.mojito.perf.instrumentMojitoRequest = instrumentMojitoRequest;
        Y.mojito.perf._dump = _dump;
