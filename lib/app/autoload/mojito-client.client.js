...
YUI.add('mojito-client', function(Y, NAME) {
    ...
    var PAUSEABLE = [
        ...
        DEPLOY_YUI_PREFIX = '$y_',
        DEPLOY_MOJITO_PREFIX = '$m_',
    ...
    /**
     * Decompresses the payload for binders
     *
     * The "config" {Object} will be modified.
     *
     * @return {Object} the modified config
     */
    function decompressBinders(config) {
        var yuiBase,
            yuiPrefix,
            mojitoPrefix,
            mojitoStaticPrefix,
            binderMap,
            deployConfig, // config.appConfig.deployRuntimeClient
            client, // config.deployRuntimeClient
            //
            viewId,
            module,
            binder,
        ...
        if (!config ||
                Y.Lang.isUndefined(config.binderMap) ||
                0 === Y.Object.size(config.binderMap)) {
            return config;
        }
        client = (config &&
                    config.deployRuntimeClient &&
                    Y.Lang.isObject(config.deployRuntimeClient) &&
                    config.deployRuntimeClient) || undefined;
        client = Y.Object.getValue(config, ['deployRuntimeClient']);
        if (!client) {
            // NOTE: default is to return config "as is"
            //Y.log('Client runtime decompressing binders, but missing meta data',
            //      'error', NAME);
            return config;
        }
        deployConfig = Y.Object.getValue(config, ['appConfig', 'deployRuntimeClient']) || {};
        if (Y.Lang.isUndefined(deployConfig.yuiConfigBase) ||
                !Y.Lang.isString(deployConfig.yuiConfigBase)) {
            Y.log('Missing yuiConfigBase value to write module path', 'error', NAME);
            return config;
        }
        if (Y.Lang.isUndefined(client.optimized) ||
                !Y.Lang.isObject(client.optimized) ||
                0 === Y.Object.size(client.optimized)) {
            Y.log('Missing metadata to decompress binders', 'error', NAME);
            return config;
        }

        binderMap = config.binderMap;
        yuiBase = deployConfig.yuiConfigBase;
        yuiPrefix = deployConfig.yuiPrefix || DEPLOY_YUI_PREFIX;
        mojitoPrefix = deployConfig.mojitoPrefix || DEPLOY_MOJITO_PREFIX;
        mojitoStaticPrefix = deployConfig.staticHandlingPrefix || DEPLOY_MOJITO_STATIC_PREFIX;

        hashMap = client.optimized;
            ...
            if (binderMap.hasOwnProperty(viewId)) {
                ...
                for (module in binder.needs) {
                    if (binder.needs.hasOwnProperty(module)) {
                        path = hashMap[module];
                        if (path) {
                            if (yuiBase && -1 !== path.indexOf(yuiPrefix)) {
                                path = path.replace(yuiPrefix, yuiBase);
                            } else if (-1 !== path.indexOf(mojitoPrefix)) {
                                path = path.replace(mojitoPrefix, mojitoStaticPrefix);
                            }
                            binder.needs[module] = path;
                        }
                    }
                }
            }
        }
        delete config.deployRuntimeClient;
        Y.log('Y.mojito.Client.decompressBinders OK', 'debug', NAME);
    ...
    MojitoClient.prototype = {
         ...
         * @return {Object}
         */
        decompressBinders: function(config) {
            return decompressBinders(config);
        },
