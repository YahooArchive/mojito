...
YUI.add('mojito-client', function(Y, NAME) {
    ...
    function privateRender(mp, data, template, cb) {
        var mojitTemplate,
            renderer;

        if (!mp._templates || !mp._templates[template]) {
            cb(new Error('Template "' + template + '" not found'));
            return;
        }
        ...
        mojitTemplate = mp._templates[template];
        renderer = new Y.mojito.TemplateRenderer(mojitTemplate.engine);
        Y.log('Rendering "' + template + '" in Binder', 'debug', NAME);
        renderer.render(data, mp.type, mojitTemplate['content-path'], {
            buffer: '',
            flush: function(data) {
                this.buffer += data;
            },
    ...
    function setNewMojitTemplate(templateData, mp) {

        Y.log('setting new template on mojit ' + mp._instanceId, 'debug', NAME);

        var newNode = Y.Node.create(templateData);
        mp._node.replace(newNode);
        mp._element = document.getElementById(mp._templateId);
        mp._node = new Y.Node(mp._element);

        if (Y.Lang.isFunction(mp._binder.onRefreshTemplate)) {
            mp._binder.onRefreshTemplate(mp._node, mp._element);
        }
    }

    // we have to match the children by instanceId to identify them and replace
    // their templateId with the actual templateId for the binder that should be
    // attached.
    function processChildren(children, binderMap) {
        ...
        for (name in children) {
            if (children.hasOwnProperty(name)) {
                for (templateId in binderMap) {
                    if (binderMap.hasOwnProperty(templateId) && !found) {
                        if (binderMap[templateId].instanceId ===
                                children[name].instanceId) {
                            children[name].templateId = templateId;
                            found = true;
                        }
                    }
                }
     ...
     *  <dd>[optional] the parent binder template id to attach any children</dd>
     *  <dt><code>topLevelMojitTemplateId</code></dt>
     *  <dd>[optional] the topmost (root) binder template id to attach as a child to
     *      the parent</dd>
     * </dl>
     *
    ...
    MojitoClient.prototype = {
         ...
         * @private
         * @param {Object} binderMap templateId ==> binder data, contains all we
         ...
         *     the document.
         * @param {string} parentId the parent binder template id to attach any
         *     children.
         * @param {string} topLevelMojitTemplateId the topmost (root) binder template
         *     id to attach as a child to the parent.
         */
        attachBinders: function(binderMap, parentId, topLevelMojitTemplateId) {
            var context = this.context,
                ...
                bindersComplete = 0,
                onBinderComplete,
                // Note: This here so we can get access template meta data for
                // each binder
                store = this.resourceStore,
                eventData = {
                    binderMap: binderMap,
                    parentId: parentId,
                    topLevelMojitTemplateId: topLevelMojitTemplateId
                };
            ...
            topLevelMojitTemplateId = eventData.topLevelMojitTemplateId;
            ...
            onBinderComplete = function() {
                ...
                // handles
                Y.Array.each(newMojitProxies, function(item) {
                    var templateid = item.proxy.getId(),
                        mojit = me._mojits[templateid],
                        proxy = item.proxy;

                    mojit.handles = bindNode(proxy._binder, proxy._node,
                        proxy._element);

                    recordBoundMojit(me._mojits, parentId, templateid, proxy.type);
                });
            ...
            Y.Object.each(binderMap, function(binderData, templateId) {

                // Make sure templateIds's are not bound to more than once
                if (me._mojits[templateId]) {
                    Y.log('Not rebinding binder for ' + binderData.type +
                          ' for DOM node ' + templateId, 'info', NAME);
                    onBinderComplete();
                    return;
                }
                ...
                YUI._mojito.loader.load(binderData.needs, function(err) {
                    ...
                    Y.use(binderData.name, function(BY) {
                        ...
                        // Check again to make sure templateIds's are not bound
                        ...
                        if (!element) {
                            Y.log('Did not find DOM node "' + templateId +
         ...
         * @param {String} templateId the template id of the current mojit, which is
         *     executing the action.
         * @param {Function} cb callback to run when complete.
         * @private
         */
        executeAction: function(command, templateId, cb) {
            var my = this;
                ...
                command.instance, this.context, function(err, details) {
                    ...
                    if (!existsOnClient || command.rpc) {
                        ...
                        my.tunnel.rpc(command,
                            new Y.mojito.OutputHandler(templateId, cb, my));
                    } else {
                        // Dispatch locally
        ...
        doRender: function(mp, data, template, cb) {
            if (!mp._templates || !mp._assetsRoot) {
                this.resourceStore.getType('client', mp.type, mp.context,
                    function(err, typeInfo) {
                        ...
                        mp._templates = typeInfo.templates;
                        mp._assetsRoot = typeInfo.assetsRoot;
                        privateRender(mp, data, template, cb);
                    });
            } else {
                privateRender(mp, data, template, cb);
            }
        ...
        doBroadcast: function(eventId, source, payload, opts) {
            ...
            if (opts && opts.target) {
                ...
                } else if (opts.target.templateId) {
                    tgtTemplateId = opts.target.templateId;
                }
            }
            if (this._listeners[eventId]) {
                Y.Array.each(this._listeners[eventId], function(listener) {
                    if (!tgtTemplateId || tgtTemplateId === listener.templateId) {
                        listener.cb({
                            data: payload,
                            source: source
                        });
        ...
        doListen: function(eventId, templateId, callback) {
            if (!this._listeners[eventId]) {
                this._listeners[eventId] = [];
            }
            this._listeners[eventId].push({
                guid: templateId,     // DEPRECATED, use templateId instead
                templateId: templateId,
                cb: callback
            });
        },
        ...
        doUnlisten: function(templateId, needleEvent) {
            ...
            if (needleEvent) {
                processListenerArray(listeners[needleEvent], templateId);
            } else {
                ...
                for (eventType in listeners) {
                    if (listeners.hasOwnProperty(eventType)) {
                        processListenerArray(listeners[eventType], templateId);
                    }
                }
            }
        },
        ...
        refreshMojitTemplate: function(mp, opts, cb) {
            var my = this;
            ...
            mp.invoke(mp._action, opts, function(err, data, meta) {
                 ...
                 * DOM stuff, we are going to replace all the new template ids with
                 * our current template ids for this mojit template as well as any
                 * children that have come along for the ride.
                 */

                var idReplacements = {}, // from: to
                    metaBinderTemplateId,
                    mBinder,
                    freshBinders = {},
                    clientMojitTemplateId,
                    clientMojit,
                    processMojitChildrenForIdReplacements;

                /*
                 * Recursive function used to walk down the hierarchy of
                 * children in order to replace every template id within the meta
                 * data
                 */
                processMojitChildrenForIdReplacements =
                            ...
                            if (metaChildren.hasOwnProperty(slot)) {
                                ...
                                if (childMojitProxy) {
                                    metaSubChildren = meta.binders[
                                        metaChild.templateId
                                    ].config.children;
                                    idRepls[metaChild.templateId] =
                ...
                for (clientMojitTemplateId in my._mojits) {
                    if (my._mojits.hasOwnProperty(clientMojitTemplateId)) {
                        clientMojit = my._mojits[clientMojitTemplateId];
                        for (metaBinderTemplateId in meta.binders) {
                            if (meta.binders.hasOwnProperty(metaBinderTemplateId)) {
                                mBinder = meta.binders[metaBinderTemplateId];
                                if (mBinder.instanceId ===
                                    ...
                                    idReplacements[metaBinderTemplateId] =
                                        clientMojitTemplateId;
                                    processMojitChildrenForIdReplacements(
                ...
                Y.Object.each(meta.children, function(child, slot) {
                    ...
                    var childTemplateId = idReplacements[child.templateId],
                        childMojit = my._mojits[childTemplateId],
                        childProxy,
                        childNode,
                        childElement,
                        childBinder;
                    ...
                    if (!childMojit) {
                        ...
                        // instantiate
                        freshBinders[child.templateId] = meta.binders[child.templateId];
                        return;
                    }

                    childProxy = my._mojits[childTemplateId].proxy;
                    childNode = mp._node.one('#' + childTemplateId);
                    ...
                    if (Y.Lang.isFunction(childBinder.onRefreshTemplate)) {
                        childBinder.onRefreshTemplate(childNode, childElement);
