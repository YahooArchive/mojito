    ...
    function dispatch(command, adapter) {
        ...
        store.store.expandInstance(command.instance, command.context, function(err, instance) {
            ...
            yuiModules.push(function (MOJIT_Y) {
                ...
                // }
                controller = CACHE.controllers[command.controllerCacheKey];
                if (undefined === controller) {
                    controller = MOJIT_Y.mojito.controllers[command.instance['controller-module']];
                    CACHE.controllers[command.controllerCacheKey] = controller;
                    console.log('Created new controller for type : ' + command.instance.type);
                } else {
                    console.log('Using cached controller for type : ' + command.instance.type);
                }
