    ...
    */
    ...
    function dispatch(command, adapter) {
        ...
        store.store.expandInstance(command.instance, command.context, function(err, instance) {
            ...
            // Abstract the key generation in mojito-util is already loaded,
            // cost should be rather inexpensive. Both keys are same, but don't
            // have to be
            yuiCacheKey = Y.mojito.util.createCacheKey(command.context);
            // controllerCacheKey = Y.mojito.util.createCacheKey(Y.mix(command.context, { type: instance.type }));
            // FIXME: cache key based on context does not work
            controllerCacheKey = instance.type;
            // FIXME: not sure why these are not written to intance ??
            instance.controllerCacheKey = controllerCacheKey;
            instance.yuiCacheKey = yuiCacheKey;
            ...
            yuiObj = CACHE.YUI[yuiCacheKey];
            if (undefined === yuiObj) {
                yuiObj = YUI();
                CACHE.YUI[yuiCacheKey] = yuiObj;
            }
            yuiObj.applyConfig(yuiConfig);
            ...
            yuiModules.push(function (MOJIT_Y) {
                ...
                // }
                controller = CACHE.controllers[controllerCacheKey];
                if (undefined === controller) {
                    controller = MOJIT_Y.mojito.controllers[instance['controller-module']];
                    CACHE.controllers[controllerCacheKey] = controller;
                }
                ...
                // Don't use init, use an addon to access configs
                ...
                ac = new MOJIT_Y.mojito.ActionContext({
                    ...
                    // models: this.models, // can be accessed via addon
                    dispatch: dispatch,
                    adapter: adapter,
                    store: store
                });
        ...
        // perf = Y.mojito.perf.timeline('mojito', 'dispatch:expandInstance',
        //     'gather details about mojit', perfID);
        ...
        //         // once the instance is expanded, we can describe it better
        //         perfID = Y.mojito.perf.idFromCommand(command);
        ...
        //                     NAME);
        //                 perfCC = Y.mojito.perf.timeline('mojito', 'cc:ctor', 'create ControllerContext', perfID);
        ...
        //                     'dispatching an instance', perfID);
        ...
        //             perf = Y.mojito.perf.timeline('mojito', 'dispatch:Y.use', instance.yui.sorted, perfID);
