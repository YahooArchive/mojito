...
YUI.add('mojito-route-maker', function(Y, NAME) {
     ...
     * @param {Boolean} init the routesCache if true
     */
    function Maker(routes, init) {
        ...
        for (name in routes) {
            if (routes.hasOwnProperty(name)) {
                if (!routesCache[name]) {
                    routesCache[name] = buildRoute(name, routes[name]);
                }
            }
        }
    }
    ...
    Maker.prototype = {
        ...
        make: function(query, verb) {
            ...
            route = this._matchToExternal(call, params, verb, routesCache);
        ...
        find: function(uri, verb) {
            ...
            route = this._matchToInternal(uri, verb, routesCache);
            if (!route) {
                return null;
            }
