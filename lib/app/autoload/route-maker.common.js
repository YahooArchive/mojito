...
YUI.add('mojito-route-maker', function (Y, NAME) {
    ...
    **/
    function Maker(routes, init) {
        ...
        // TODO: Cache these computed routes so that its not done on every
        // request ?
        //
        // Collect the keys that are required
        Y.Object.each(routes, function (rr) {
            Y.Array.each(rr, function (route) {
                var requires = [];
                Y.Array.each(route.keys, function (key) {
                    requires.push(key.name);
                });
        ...
        make: function(query, verb, params) {
            ...
            for (k in params) {
                if (params.hasOwnProperty(k)) {
                    if (route.requires && route.requires.indexOf(k) > -1) {
