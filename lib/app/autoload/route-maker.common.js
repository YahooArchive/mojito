    ...
    Maker.prototype = {
        ...
        **/
        find: function(uri, verb) {
            ...
            Y.log('The method `find()` has been deprecated.', 'warn', NAME);
            ...
            Y.log('Route not found by name for path: ' + uri, 'debug', NAME);

            // We tried to look up by path, and then name, but nothing.
            //
            // So let's go through the routes and try find a match.
            //
            Y.Object.some(routes, function (route, key) {
                var regex;

                // The "#" should match the encoder in
                // "Y.mojito.util.encodeRouteName()"
                ...
                if (regex.test(key)) {
                    if (route.path.indexOf(':') > -1) {
                        if (route.regexp.test(uri)) {
                            ...
                            match = resolveRouteParams(Y.mojito.util.copy(route), uri);
                            return true;
                        }
                    }
                }
