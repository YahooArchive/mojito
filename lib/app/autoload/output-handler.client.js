...
YUI.add('mojito-output-handler', function(Y, NAME) {
    ...
    attachAssets = function(assets, cb) {
        var toLoad = {
            ...
            blobNode = '',
            doneChecker,
            loadedcssele = document.getElementsByTagName("link");
        ...
        Y.Object.each(assets, function(types, location) {
            Y.Object.each(types, function(list, type) {
                ...
                } else if (type === 'css' && loadedcssele.length !== 0) {
                    for (i = 0; i < list.length; i += 1) {
                        for (j = 0; j < loadedcssele.length; j += 1) {
                            if (loadedcssele[j].getAttribute("href").indexOf(list[i]) === -1
                                    && !loaded[type][list[i]]) {
