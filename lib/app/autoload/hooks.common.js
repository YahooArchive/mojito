 ...
 * <li>Y.mojito.hooks.hook('mojitoClient', Y.mojito.hooks.client_ctx, 'start', this);
 * <li>Y.mojito.hooks.hook('mojitoClient', Y.mojito.hooks.client_ctx, 'end', this);
 * <li>Y.mojito.hooks.hook('mojitoClientBind', Y.mojito.hooks.client_ctx, 'start', this);
 * <li>Y.mojito.hooks.hook('mojitoClientBind', Y.mojito.hooks.client_ctx, 'resume', this);
 * <li>Y.mojito.hooks.hook('mojitoClientBind', Y.mojito.hooks.client_ctx, 'end', this);
 * <li>Y.mojito.hooks.hook('mojitoClientBindComplete', Y.mojito.hooks.client_ctx, 'start', this);
 * <li>Y.mojito.hooks.hook('mojitoClientBindComplete', Y.mojito.hooks.client_ctx, 'end', this);
 * </ul>
 * @module MojitoHooks
 */
...
YUI.add('mojito-hooks', function(Y, NAME) {
     ...
     * Example:
     * <pre>
     * Y.mojito.hooks.hook('test_hook', ctx, data1);
     * </pre>
     */
    ...
    Y.mojito.hooks.real_hook = function (hook, ctx) {
        ...
        if (!ctx || !map_hook_tool[hook]) {
            return;
        }
        ds_args = [].slice.apply(arguments).slice(2);
        for (tool in map_hook_tool[hook]) {
            if (map_hook_tool[hook].hasOwnProperty(tool) && ctx[tool]) {
    ...
    Y.mojito.hooks.registerHook = function (tool, hook, cb) {
        ...
        Y.log("register tool: " + tool + ", hook: " + hook);
    };
    ...
    Y.mojito.hooks.enableHookGroup = function (req, tool) {
        ...
        if (!req) {
            if (!Y.mojito.hooks.client_ctx) {
                Y.mojito.hooks.client_ctx = {};
            }
            if (!Y.mojito.hooks.client_ctx[tool]) {
                Y.mojito.hooks.client_ctx[tool] = {};
            }
            ret = Y.mojito.hooks.client_ctx[tool];
        } else {
            if (!req.hook) {
                req.hook = {};
            }
