...
YUI.add('mojito-hooks', function(Y, NAME) {
    ...
    Y.mojito.hooks.real_hook = function (hook, ctx) {
        ...
        if (!map_hook_tool[hook] || (!ctx && !Y.mojito.hooks.client_ctx)) {
            return;
        }
        ...
        for (tool in map_hook_tool[hook]) {
            if (map_hook_tool[hook].hasOwnProperty(tool)) {
                ...
                if (Y.mojito.hooks.client_ctx && Y.mojito.hooks.client_ctx[tool]) {
                    map_hook_tool[hook][tool].apply(Y.mojito.hooks.client_ctx[tool], ds_args);
                }
            }
        }
    };
