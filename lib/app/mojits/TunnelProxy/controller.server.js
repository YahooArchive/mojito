    ...
    function makeAdapter(ac) {
        ...
        newAdapter = Y.mix(oldAdapter, {
            ...
            _updateBuffer: function(data, meta) {
                ...
                // metaMerge will strip off the template info, but we need that for
                // RPC calls, so we put it back
                if (meta.template) {
                    if (buff.meta.template) {
                        buff.meta.template = Y.mojito.util.metaMerge(
                            buff.meta.template,
                            meta.template
                        );
                    } else {
                        buff.meta.template = meta.template;
                    }
                }
            },
