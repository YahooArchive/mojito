...
exports.buildhtml5app = function(cmdOptions, store, config, destination,
    ...
    var loaderModules = ['yui-base.js', 'loader-base.js', 'loader.js', 'loader-yui3.js', 'loader-app-base.js'];
    for (mr = 0; mr < loaderModules.length; mr += 1) {
        // These have to be placed under 'static'
        urls[loaderModules[mr]] = loaderModules[mr];
    }
    // TODO: "static" should match value from staticHandling config
    // Copy the YUI bundle so that it is available offline
    sourceYUI = findYUIDir();
    destinationYUI = libpath.join(destination, 'static', 'yui', 'build');
    mkdirP(destinationYUI, MODE_755);
    libwrench.copyDirSyncRecursive(sourceYUI, destinationYUI);
...
/**
 * Locate the directory where YUI is installed within "mojito" module
 */
findYUIDir = function() {
    var source;
    source = libpath.join(__dirname, '../../../node_modules', 'yui');
    return source;
};
...
function attachCharset(root, relativePath, content, force) {
    var extname = libpath.extname(relativePath);
    ...
    if (force || '.html' === extname) {
        ...
        content = content.replace(/(src|href)="([^"]+)"/g,
            function(all, name, val) {
                ...
                // Causing all the /, & and = to be HTML encoded. Those are URLs,
                // no need to do so. Just URLEncode is good enough,
...
writeWebPagesToFiles = function(destination, urls, config, callback) {
    ...
    app.listen(null, null, function(err) {
        ...
        Object.keys(urls).forEach(function(u) {
            ...
            // if (! /^\//.test(u)) {
            if ('mojito-mu.js' === u) {
                // force all mojito specific stuff to load from /static
                // HACK: returns the "client" affinity for mojito-mu
                u2 = libpath.join('/static', u);
            } else {
                u2 = libpath.join('/', u);
            }

            app.getWebPage(u2, opts, function(err, url, content) {
                var dest;
                ...
                if (err) {
                    ...
                    if (config.attachManifest) {
                        content = attachManifest(destination, urls[u],
                            content,
                            userPages[u]);
                    }
                    // TODO: expose as configuration?
                    if (true /*config.attachCharset*/) {
                        content = attachCharset(destination, urls[u], content, userPages[u]);
                    }
                    if (config.forceRelativePaths) {
                        content = forceRelativePaths(destination, urls[u],
                            content,
                            userPages[u]
                            );
                    }
                    // HACK: need meta from store to determine this
                    // if no /static/ add it
                    // FIXME: "static" should  match staticHandling
                    if ('/' === u) {
                       dest = libpath.join(destination, urls[u]);
                    } else if (! /\/static/.test(urls[u])) {
                        dest = libpath.join(destination, 'static', urls[u]);
                    } else {
                        dest = libpath.join(destination, urls[u]);
                    }
