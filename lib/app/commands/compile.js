...
usage = 'mojito compile {options} {type}\n' +
    ...
    'them into JS\n' +
    '\ttemplates  precompiles the templates to speed rendering\n' +
...
compile.all = function(context, options, callback) {
    ...
    if (options.remove) {
        actions = [
            ...
            // mojits that JUST have compiled inlinecss or templates
            'rollups',
            'json',
            'inlinecss',
            'templates'
...
/**
 * Compiles templates for the application.
 * @param {object} context The context.
 * @param {object} options Command options.
 * @param {function} callback Function to invoke upon completion.
 * @return {object} The return value from any optional callback function.
 */
compile.templates = function(context, options, callback) {
    var self = this,
        cwd = process.cwd(),
        store = makeStore({root: cwd}),
        compiledFilename = '/autoload/compiled/templates.common.js',
        mojits,
        yuiConfig,
        renderer,
        renderedTemplate,
    ...
    // there are no templates in the app, so no need to do this
    if (options.app) {
        libutils.warn('Compiling app-level templates not supported\n');
        return callback();
    }
    ...
    Y.Array.each(mojits, function(mojitName) {
        ...
        outputFilepath = libpath.join(mojitRes.source.fs.fullPath, 'autoload/compiled/templates.common.js');

        if ('mojito' === mojitRes.source.pkg.name) {
            // don't write framework-provided templates into the framework directory
            return;
        }
        ...
        yuiModuleCacheWriter = new YuiModuleCacheWriter('templates/' + mojitName,
            outputFilepath, options);

        mojit = store.getMojitTypeDetails('server', context, mojitName);

        if (mojit.templates) {
            compileFunction = function (renderer, source, mojitNs, templateName) {
                renderer.compiler(source, function (err, templateObj) {
                    renderedTemplate = Y.JSON.parse(templateObj.toString());
                    yuiModuleCacheWriter.createNamespace('compiled.' +
                        mojitNs + '.templates').cache(templateName,
                        renderedTemplate);
                });
            };
            // Check each template for a template and engine
            for (templateName in mojit.templates) {
                if (mojit.templates.hasOwnProperty(templateName)) {
                    template = mojit.templates[templateName];
                    // Check if there is a template to build
                    if (template['content-path'] && template.engine) {
                        if (!mojitTemplates[mojitName]) {
                            mojitTemplates[mojitName] = {};
                        }
                        // We have a template to compile
                        source = template['content-path'];
                        engine = template.engine;
                        yuiConfig = mojit.yui.config;
                        yuiConfig.useSync = true;

                        MojY = YUI(yuiConfig).use('mojito-' + engine);
                        renderer = new (MojY.mojito.addons.templateEngines[engine])();

                        if (typeof renderer.compiler === 'function') {
                            compilerQueue.add(Y.bind(compileFunction, self, renderer, source, mojitNs, templateName));
                        }
                    }
                }
            }
    ...
    compilerQueue.add(function () {
        msgs.push(action + ' compiled template YUI modules for ' + processed +
            ' mojits.');
        callback();
    });
    compilerQueue.run();
