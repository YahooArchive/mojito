...
function getArchetypeSrcDir(type, subtype) {
    var stat,
        srcdir = type === 'custom' ?
            path.resolve(subtype) : path.resolve(archetypePath, type, subtype);

    try {
        stat = fs.statSync(srcdir);
        if(!stat.isDirectory()) {
            utils.error(
                'Archetype path is not a directory: ' + srcdir, usage, true);
        }
    } catch(err) {
        utils.error('Archetype path does not exist: ' + srcdir, usage, true);
    }
...
function cleanName(name) {
    var newname = name && name.replace(/[^a-z0-9]+/ig, '_'),
        msg;

    if (!name) {
        utils.error('Missing a target name to create', usage, true);
    }

    if (-1 !== reservedWords.indexOf(newname)) {
        msg = 'Name cannot be one of: ' + reservedWords.join(', ') + '\n';
        utils.error(msg, usage, true);
    }

    if (name !== newname) {
        msg = ['changing name ', name, ' to ', newname,
            ' so it is usable as a javascript identifier: ', null].join('"');
        utils.log(msg);
    }
...
function run(params, options, callback) {
    ...
    var type = params[0] ? params[0].toLowerCase() : '',
        subtype = params[1],
        name = params[2],
    ...
    // If we have no name then the "subtype" may be it
    if (!name) {
        name = subtype;
        subtype = 'default';
    }
    ...
    case 'custom':
        ...
        break;
    case 'mojit':
        destdir = path.resolve(process.cwd(), name, 'mojits');
        break;
    default:
        return utils.error(
            'Incorrect type "' + type +
            '", must be either "app", "mojit", or "custom".', usage);
    }
