...
function exec(conf, store, scraper) {
    var buildmap = {};
    ...
    this.mapStoreUris(buildmap, conf, store.getAllURLs() || {});

    // add tunnel uris to buildmap
    this.mapDefxUris(buildmap, conf, store);

    // add uris to buildmap from legacy mojit ids that have spec file parts
    this.mapFunkySpecUris(buildmap, conf);

    // create a couple files
    this.makeManifest(buildmap, conf.build.dir, null);

    // use buildmap to scrape mojito server and save the rendered pages
    scraper
        .on('scraped-one', this.mungePage.bind(this, conf))
