    ...
    Y.extend(RSAddonYUI, Y.Plugin.Base, {
        ...
        onGetMojitTypeDetails: function(evt) {
            ...
            for (r = 0; r < ress.length; r += 1) {
                res = ress[r];
                if (res.type === 'binder') {
                    if (!dest.templates[res.name]) {
                        dest.templates[res.name] = {};
                    }
                    dest.templates[res.name]['binder-module'] = res.yui.name;
                    sorted = this._getYUIDependencies('client', poslKey, ctx.lang, res.yui.name);
                    if (sorted && sorted.paths) {
                        dest.templates[res.name]['binder-yui-sorted'] = Y.clone(sorted.paths, true);
                    }
                }
        ...
        onMojitResourcesResolved: function(evt) {
            ...
            for (r = 0; r < ress.length; r += 1) {
                res = ress[r];
                if ('addon' === res.type && 'template-engines' === res.subtype) {
                    templateEngineRequired[res.yui.name] = true;
                }
            ...
            for (l = 0; l < langNames.length; l += 1) {
                ...
                if (controller) {
                    ...
                    // we don't know which templates will be used, so we need all template engines
                    required = Y.merge(required, templateEngineRequired);
                ...
                if ('client' === env) {
                    for (binderName in binders) {
                        if (binders.hasOwnProperty(binderName)) {
                            ...
                            // template engines are needed to support mojitProxy.render()
                            required = Y.merge(required, templateEngineRequired);
