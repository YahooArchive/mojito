...
YUI.add('addon-rs-yui', function(Y, NAME) {
    ...
    Y.extend(RSAddonYUI, Y.Plugin.Base, {
        ...
        onMojitResourcesResolved: function(evt) {
            ...
            if (X_NEW) {
                if (controller) {
                    ...
                    this._addLangsToSorted(env, sorted, langs);
        ...
        _precomputeYUIDependencies: function(lang, env, mojit, modules, required, forceYLoader) {
            ...
            if ( (!this.usePrecomputed) && (!forceYLoader) ) {
        ...
        _setYUIDependencies: function(env, poslKey, lang, module, sorted) {
            ...
            if (X_NEW) {
                this.sortedModules[env][poslKey][module] = sorted;
                return;
            }
        ...
        // add lang bundles to sorted
        _addLangsToSorted: function(env, sorted, langs) {
            Y.Object.each(langs, function(res, langName) {
                sorted.sorted.push(res.yui.name);
                sorted.paths[res.yui.name] = ('client' === env) ? res.url : res.source.fs.fullPath;
            });
        ...
        // add lang to sorted
        _addLangToSorted: function(env, sorted, res) {
            if (res) {
                sorted.sorted.push(res.yui.name);
                sorted.paths[res.yui.name] = ('client' === env) ? res.url : res.source.fs.fullPath;
            }
