        ...
        getAppSeedFiles: function(ctx) {
            ...
            for (i = 0; i < seed.length; i += 1) {
                if (hash.hasOwnProperty(seed[i])) {
                    ...
                    if (this.yuiModulesDetails.hasOwnProperty(files[i])) {
                        files[i] = this.yuiModulesDetails[files[i]].url;
                    } else if (this.appModulesDetails.hasOwnProperty(files[i])) {
                        files[i] = this.appModulesDetails[files[i]].url;
                    }
                }
                ...
                for (r = 0; r < ress.length; r += 1) {
                    ...
                    if (me.appModulesDetails[res.yui.name]) {
                        Y.log('Language bundle collision for name=' + res.yui.name +
                              '. Choosing: ' + me.appModulesDetails[res.yui.name].path +
