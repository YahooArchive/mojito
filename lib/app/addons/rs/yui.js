...
YUI.add('addon-rs-yui', function(Y, NAME) {
    ...
    Y.extend(RSAddonYUI, Y.Plugin.Base, {
         ...
         * from `application.json`. This config IS based on the
         * static context.
         * @method getYUIConfig
        ...
        getYUIConfig: function() {
            ...
            if (this.staticHandling.serveYUIFromLocal) {
            ...
            yuiConfig.groups.app = this.getAppGroupConfig();
         ...
         * The `seed` IS based on the static context, but lang
         * is based on the req context.
        ...
        getAppSeedFiles: function(ctx, yuiConfig) {
            ...
            for (i = 0; i < seed.length; i += 1) {
                ...
                seed[i] = seed[i].replace(REGEX_LANG_PATH, closestLang);
                ...
                if (resolvedMods.hasOwnProperty(m)) {
                    ...
                    if (appModules[module.name]) {
                        ...
                        module.path = module.path.split('/').pop()
                            // and removing the -min or -debug just in case
                            .replace('-min.', '.').replace('-debug.', '.');
