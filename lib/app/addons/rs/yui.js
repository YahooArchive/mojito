    ...
    Y.extend(RSAddonYUI, Y.Plugin.Base, {
        ...
        makeResourceVersions: function() {
            ...
            for (l = 0; l < langs.length; l += 1) {
                ...
                res = {
                    source: {},
                    type: 'yui-loader',
                    subtype: 'app-resolved',
                    name: lang,
                    affinity: 'common',
                    selector: '*',
                    yui: {
                        name: 'loader-app-resolved' + langExt
                    }
                };
                res.id = [res.type, res.subtype, res.name].join('-');
                res.source.pkg = store.getAppPkgMeta();
                res.source.fs = store.makeResourceFSMeta(this.appRoot, 'app', '.', 'loader-app-resolved' + langExt + '.js', true);
                store.addResourceVersion(res);
                ...
                res = {
                    ...
                    selector: '*',
                    yui: {
                        name: 'loader-yui3-base' + langExt
                    }
                };
                res.id = [res.type, res.subtype, res.name].join('-');
                res.source.pkg = store.getAppPkgMeta();
                res.source.fs = store.makeResourceFSMeta(this.appRoot, 'app', '.', 'loader-yui3-base' + langExt + '.js', true);
                store.addResourceVersion(res);

                res = {
                    source: {},
                    type: 'yui-loader',
                    subtype: 'yui3-resolved',
                    name: lang,
                    affinity: 'common',
                    selector: '*',
                    yui: {
                        name: 'loader-yui3-resolved' + langExt
                    }
                };
                res.id = [res.type, res.subtype, res.name].join('-');
                res.source.pkg = store.getAppPkgMeta();
                res.source.fs = store.makeResourceFSMeta(this.appRoot, 'app', '.', 'loader-yui3-resolved' + langExt + '.js', true);
                store.addResourceVersion(res);
            }
        },
        ...
        _precalcLoaderMeta: function(langs) {
            ...
            for (i = 0; i < langs.length; i += 1) {
                ...
                this.resContents['yui-loader-app-resolved-' + lang] =
                    this._produceMeta('loader-app-resolved', lang || '*', appMetaData, yuiMetaData);

                this.resContents['yui-loader-yui3-base-' + lang] =
                    this._produceMeta('loader-yui3-base', lang || '*', appMetaData, yuiMetaData);

                this.resContents['yui-loader-yui3-resolved-' + lang] =
                    this._produceMeta('loader-yui3-resolved', lang || '*', appMetaData, yuiMetaData);
            }
        },
