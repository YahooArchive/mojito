    ...
    Y.extend(RSAddonYUI, Y.Plugin.Base, {
        ...
        getModulesConfig: function(env, justApp) {
            ...
            for (m = 0; m < mojits.length; m += 1) {
                mojit = mojits[m];
                ress = store.getResourceVersions({ mojit: mojit, chosen: true });
                for (r = 0; r < ress.length; r += 1) {
                    res = ress[r];
                    if (!res.yui || !res.yui.name) {
                        continue;
        ...
        resolveResourceVersions: function() {
            ...
            for (m = 0; m < mojits.length; m += 1) {
                mojit = mojits[m];
                process(store.getResourceVersions({ mojit: mojit, chosen: true }));
            }
        ...
        _processMeta: function(resolvedMods, modules, expanded_modules, conditions, appModules) {
            ...
            for (m in resolvedMods) {
                if (resolvedMods.hasOwnProperty(m)) {
                    ...
                    if (lang) {
                        ...
                        mod1 = mod.split("/");
                        add = true;
                        for (i = 0; i < intlmodules.length; i += 1) {
                            if (intlmodules[i] === mod1[1]) {
                                add = false;
                            }
                        }
                        if (add) {
            ...
            for (m in resolvedMods) {
                if (resolvedMods.hasOwnProperty(m)) {
                    module = resolvedMods[m];
                    for (i = 0; i < intlmodules.length; i += 1) {
                        if (module.name === intlmodules[i]) {
                            modules[module.name] = {};
                            for (i = 0; i < MODULE_META_ENTRIES.length; i += 1) {
                                if (MODULE_META_ENTRIES[i] === 'lang') {
