...
YUI.add('addon-rs-config', function(Y, NAME) {
    ...
    Y.extend(RSAddonConfig, Y.Plugin.Base, {
        ...
        readConfigThroughCacheSync: function(fullPath) {
            var json;
        ...
        readConfigSync: function (filePath, type) {
            ...
            try {
                raw = libfs.readFileSync(filePath + file[type], 'utf8');
                try {
                ...
                if (err.errno !== 34) { // if the error was not "no such file or directory" say something
                    throw new Error("Error parsing JSON file: " + filePath + file[type] + "\n" + err);
                }
        ...
        readConfigYCB: function(fullPath, ctx) {
            ...
            if (!ycb) {
                json = this.readConfigThroughCacheSync(fullPath);
        ...
        _readYcbDimensions: function() {
            ...
            return this.readConfigThroughCacheSync(path);
        }
