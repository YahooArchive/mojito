...
YUI.add('addon-rs-config', function(Y, NAME) {
    ...
    Y.extend(RSAddonConfig, Y.Plugin.Base, {
        ...
        readConfigSync: function (filePath, type) {
            ...
            try {
                raw = libfs.readFileSync(filePath + extensions[type], 'utf8');
                try {
                ...
                if (err.errno !== 34) { // if the error was not "no such file or directory" report it
                    throw new Error("Error parsing file: " + filePath + extensions[type] + "\n" + err);
                }
                if (type < extensions.length - 1) {
                    return this.readConfigSync(filePath, type + 1);
                }
            }
            //warn if both yaml and json files are present, use yaml
            if (type === 0 || type === 1) {
                try {
                    libfs.readFileSync(filePath + extensions[2], 'utf8');
                    Y.log(filePath + extensions[2] + ' exists. But yaml file will be used', 'warn', NAME);
                } catch (error) {
                    if (error.errno !== 34) { // if the error was not "no such file or directory" report it
                        throw new Error("Error reading file: " + filePath + extensions[2] + "\n" + error);
                    }
                }
            }
            return obj;
