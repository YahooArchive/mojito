...
YUI.add('mojito-composite-addon', function(Y, NAME) {
    ...
    */
    ...
    Addon.prototype = {
         ...
         * their executed values into the template, keyed by the child's
...
Y.mojito.controller = {
    index: function(ac) {
        ac.composite.done({
            template: { title: 'Hello there' } // for the template only
        });
    }
        };
</pre>
         * This will execute the child intances of the "FooMojit" and
         * "BarMojit", returning their rendered values into the parent's
         * template, thus rendering the full parent template including the children.
