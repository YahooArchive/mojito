    ...
    AdapterBuffer.prototype = {
            ...
            if (this.buffer.__counter__ === 0) {
                ...
                this.callback = function () {
                    Y.log('ac.error/done called multiple times in child: ' +
                            this.id, 'warn', NAME);
                };
            }
        },
        ...
        flush: function(data, meta) {
            if (meta) {
                this.buffer[this.id].meta = Y.mojito.util.metaMerge(
                    this.buffer[this.id].meta,
                    meta
                );
            }
            if (data) {
        ...
        error: function(err) {
            ...
            Y.mojito.hooks.hook('adapterBuffer', this.hook, 'end', this);
            // HookSystem::EndBlock
            this.buffer[this.id].error = err;
            this.buffer.__counter__ -= 1;
            if (this.buffer.__counter__ === 0) {
                this.callback();
                this.callback = function () {
                    Y.log('ac.done/error called multiple times in child: ' +
                            this.id, 'warn', NAME);
                };
            }
        }
                ...
                function() {
                    ...
                    for (name in buffer) {
                        if (buffer.hasOwnProperty(name) &&
                            ...
                            // making the parent to fail if a child failed and
                            // it was flagged with `propagateFailure`.
                            if (buffer[name].propagateFailure && buffer[name].error) {
                                // HookSystem::StartBlock
                                Y.mojito.hooks.hook('addon', my.adapter.hook, 'end', my);
                                // HookSystem::EndBlock
                                my.adapter.error(buffer[name].error);
                                return; // breaking the execution due to an error in child
                            }
            ...
            for (childName in children) {
                if (children.hasOwnProperty(childName)) {
                    ...
                    buffer[childName] = {
                        ...
                        data: '',
                        meta: {},
                        propagateFailure: child.propagateFailure
                    };
