...
YUI.add('mojito-output-adapter-addon', function(Y, NAME) {
    ...
    /* see action-context.common.js for docs */
        ...
        // perfID = Y.mojito.perf.idFromCommand(this.command);
        // perf = Y.mojito.perf.timeline('mojito', 'ac.done',
        //     'time to execute ac.done process', perfID);
        ...
        if (mojitView) {
            ...
            if (rendererCache['hb'] === undefined) {
                grenderer = new (Y.mojito.addons.viewEngines[mojitView.engine])('');
                rendererCache['hb'] = grenderer;
            } else {
                grenderer = rendererCache['hb'];
            }
            grenderer.viewId = meta.view.id;
            grenderer.render(data, instance.type, contentPath, adapter, meta, more);
        ...
        // Y.mojito.perf.mark('mojito', 'action:stop', 'after the action', perfID);
    };
