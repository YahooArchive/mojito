...
YUI.add('mojito-deploy-addon', function (Y, NAME) {
    ...
    var CACHE = { routeMap: null };

    // TODO: define the filter to apply before shipping routes to client
    /**
    @param {Object} appRoutes
    @param {Object} options for future
    **/
    function getRouteMap(appRoutes, options) {
        var matches = [],
            map;

        Object.keys(appRoutes).reduce(function (routes, method) {
            appRoutes[method].forEach(function (route) {
                if (route.annotations && route.annotations.name) {
                    matches.push(route);
                }
            });
            return matches;
        ...
        map = matches.map(function (route) {
            return {
                path: route.path,
                keys: route.keys,
                regexp: route.regexp,
    ...
    Addon.prototype = {
        ...
        constructMojitoClientRuntime: function (assetHandler, binderMap) {
            ...
            if (!CACHE.routeMap) {
                CACHE.routeMap = getRouteMap(pageRoutes);
            }
            clientConfig.routes = CACHE.routeMap;
            clientConfig.page = {
