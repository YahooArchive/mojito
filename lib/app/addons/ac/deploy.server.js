...
YUI.add('mojito-deploy-addon', function (Y, NAME) {
    ...
    Addon.prototype = {
         ...
         */
        constructMojitoClientRuntime: function (assetHandler, binderMap) {debugger;
...
waterfall.start('loader');
                // Make new loaders, should only be done if there are new langs.
                store.yui.makeResourceVersions(lang);
                sharedRess = store.getMojitResourceVersions('shared');

                // These are the resolveResourceVersions calls for the yui and url RS addons,
                // which focus just on shared resources.
                store.url._processMojitResource('shared');
                store.yui._processResources(sharedRess);

                // Calculate the loader meta data.
                store.yui._precalcLoaderMeta(this.ac.context.lang);

                // Add url details for the new shared resources.
                store.getURLDetails(sharedRess, store._staticDetails);
            ...
            waterfall.end('loader');
            console.log('\n' + waterfall.getSummary());
            }
