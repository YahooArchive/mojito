    ...
    Addon.prototype = {
        ...
        constructMojitoClientRuntime: function(assetHandler, binderMap) {
            ...
            var store = this.rs,
                ...
                seed = Y.Array(appGroupConfig.seed),
                contextClient,
                appConfigClient,
                yuiConfig = {},
                yuiConfigEscaped,
            ...
            for (i = 0; i < seed.length; i += 1) {
                ...
                seed[i] = seed[i].replace(REGEX_LANG_TOKEN, (closestLang ? '_' + closestLang : ''));
            }
            ...
            // application.json->yui->config->groups->app->seed in a form of
            // a array with the list of relative paths for all seed files.
            if (appGroupConfig.combine === false) {
                ...
                assetHandler.addAsset('js', 'top', appGroupConfig.comboBase +
                    appGroupConfig.root + seed.join('&' + appGroupConfig.root));
