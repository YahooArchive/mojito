 ...
 * Template engines.
 *
 * Please see the [documentation](http://developer.yahoo.com/cocktails/mojito/docs/topics/mojito_extensions.html#template-engines).
 *
...
YUI.add('mojito-mu', function(Y, NAME) {
    ...
    MuAdapter.prototype.render = function(data,
        ...
        if (meta && meta.template && meta.template['content-path']) {
            // in this case, the template name doesn't necessarily relate to the
            // contents
            useCompiled = false;
        }
        ...
        if (useCompiled && isCached(meta, ns)) {
            // Log we are using a compiled template
            Y.log('Using a compiled template for file "' + tmpl + '"', 'mojito',
                NAME);
            // If we do just hand it to the handler.
            handler(null, {
                responseText:
                    YUI._mojito._cache.compiled[ns].templates[meta.template.name]
            });
    ...
    // TODO: refactor this into app/autoload/template-renderer.common.js?
    // (so that each template engine doesn't need to implement it itself)
    isCached = function(meta, ns) {
        ...
        // try {
        //     return YUI._mojito._cache.compiled[ns].templates[meta.template.name] !==
        //         'undefined';
        // } catch {
        //     return false;
        // }
        ...
        return meta &&
            ...
            YUI._mojito._cache.compiled[ns].templates &&
            YUI._mojito._cache.compiled[ns].templates[meta.template.name];
    };

    Y.namespace('mojito.addons.templateEngines').mu = MuAdapter;
