    ...
    attachRoutes: function (routesFiles) {
        ...
        function mid(params) {
            ...
            return function (req, res, next) {
                ...
                if (!copy) {
                    // easier way ?
                    copy = req.app.mojito.Y.mojito.util.copy;
                }
                ...
                cloned = copy(o);
                debug(inspect(cloned));
                // add the fixed params to the query object if they are not
                // there
                for (key in cloned) {
                    if (cloned.hasOwnProperty(key) && !query[key]) {
                        debug('copying params from routes configuration: %s', key);
                        query[key] = cloned[key];
                    }
                }
            ...
            if (routes.hasOwnProperty(name)) {
                ...
                for (verb in route.verbs) {
                    if (route.verbs.hasOwnProperty(verb)) {
                        debug('installing handler: [' + verb + ' ' + route.path + '] ' + name);
                        ...
                        args.push(dispatcher.dispatch(route));
