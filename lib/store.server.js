    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        getMojitTypeDetails: function(env, ctx, mojitType, dest) {
            ...
            for (r = 0; r < ress.length; r += 1) {
                ...
                if (res.type === 'binder') {
                    ...
                    dest.templates[res.name]['binder-url'] = res.url;
                    if (env === 'client') {
                        dest.templates[res.name]['binder-path'] = res.url;
                    } else {
                        dest.templates[res.name]['binder-path'] = res.source.fs.fullPath;
                    }
                }
                ...
                if (res.type === 'template') {
                    ...
                    if (env === 'client') {
                        dest.templates[res.name]['content-path'] = res.url;
                    } else {
                        dest.templates[res.name]['content-path'] = res.source.fs.fullPath;
                    }
                    dest.templates[res.name].engine = res.template.engine;
                    engines[res.template.engine] = true;
                }
            }
        ...
        parseResourceVersion: function(source, type, subtype, mojitType) {
            ...
            if ('template' === type) {
                ...
                if (baseParts.length !== 1) {
                    Y.log('invalid template filename. skipping ' + fs.fullPath, 'warn', NAME);
                    return;
                }
                res.name = this._libs.path.join(fs.subDirArray.join('/'), baseParts.join('.'));
                res.id = [res.type, res.subtype, res.name].join('-');
