            ...
            for (m = 0; m < mojits.length; m += 1) {
                ...
                for (r = 0; r < ress.length; r += 1) {
                    ...
                    if (res.yui && res.yui.name && res.affinity &&
                            (res.affinity.affinity === 'common' || res.affinity.affinity === 'client')) {

                        if (urls[res.yui.name]) {
                            Y.log('YUI Modules should have unique name per affinity. Module [' +
                                res.yui.name + '] has a duplicated definition.', 'error', NAME);
                        }
                        urls[res.yui.name] = res.source.fs.rollupPath || res.source.fs.fullPath;
