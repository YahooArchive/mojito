...
YUI.add('mojito-resource-store', function(Y, NAME) {
    ...
    var libs = {},
        ...
        DEFAULT_AFFINITIES = {
            ...
            'archetype': 'server',
            'asset': 'common',
            // TODO: according to @folta, this is needed so that binders meta-bubble
            //       we should revisit that, since binders meta are already in RS
            'binder': 'client',
            'command': 'server',
            'controller': 'server',
            'middleware': 'server',
    ...
    Y.extend(ResourceStore, Y.Base, {
        ...
        expandInstanceForEnv: function(env, instance, ctx, cb) {
            ...
            try {
                spec = Y.mojito.util.copy(this._expandSpec(env, ctx, instance));
            } catch (err) {
                return cb(err);
            }
        ...
        /**
         * Sugar method that returns all "url" metadata of all resources that
         * can be accessed from the client runtime, that includes "common" and
         * "client" affinity, where "client" has the priority.
         * @method getAllModulesURLs
         * @return {object} for all resources with a "url" metadatum, the key is
         *      that URL and the value the filesystem path
         */
        getAllModulesURLs: function() {
            ...
            for (m = 0; m < mojits.length; m += 1) {
                ...
                for (r = 0; r < ress.length; r += 1) {
                    ...
                    affinity = res.affinity && res.affinity.affinity;
                    if (res.yui && res.yui.name &&
                            (affinity === 'common' || affinity === 'client')) {

                        if (urls[affinity] && urls[affinity][res.yui.name]) {
                            Y.log('YUI Modules should have unique name per affinity. ' +
                                'Module [' + res.yui.name + '] has a duplicated ' +
                                'definition for ' + affinity, 'error', NAME);
                        }
                        urls[affinity][res.yui.name] =
                            res.source.fs.rollupPath || res.source.fs.fullPath;

                    }
                }
            }
            return Y.merge(urls.common, urls.client);
        },
