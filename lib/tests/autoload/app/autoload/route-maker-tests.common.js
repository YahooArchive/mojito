...
YUI.add('mojito-route-maker-tests', function(Y, NAME) {
    ...
    suite.add(new YUITest.TestCase({
        ...
        setUp: function() {
            routeMaker = new Y.mojito.RouteMaker(routes, true);
        },
    ...
    suite.add(new YUITest.TestCase({
        ...
        setUp: function() {
            ...
            this.routeMaker = new Y.mojito.RouteMaker(routes, true);
        },
    ...
    suite.add(new YUITest.TestCase({
        ...
        setUp: function() {
            this.routeMaker = new Y.mojito.RouteMaker(getComputedRoutes_routes, true);
        },
    ...
    suite.add(new YUITest.TestCase({
        ...
        'test cache is reset on constructor': function() {
            var maker,
                cloned, 
                route,
                route2;

            maker = new Y.mojito.RouteMaker(getComputedRoutes_routes, true);
            route = maker.getComputedRoutes()["index"];
            cloned = Y.mojito.util.copy(getComputedRoutes_routes);
            cloned.index.path = '/foo.bar';
            maker = new Y.mojito.RouteMaker(cloned, true);
            route2 = maker.getComputedRoutes()["index"];
            // console.log(route2);

            A.isTrue(route.path !== route2.path, 'route2.path mismatch');
            A.areEqual('/foo.bar', route2.path, 'wrong route2.path');
        },

        'test cache hit on 2nd query for same route': function() {
            var maker,
                cloned, 
                route,
                route2;
            ...
            cloned = Y.mojito.util.copy(getComputedRoutes_routes);
            cloned.index.path = '/foo.bar';
            maker = new Y.mojito.RouteMaker(cloned);
            route2 = maker.getComputedRoutes()["index"];

            A.areEqual(route.path, route2.path, 'cache was corrupted');
            A.areEqual(route.call, route2.call, 'call was corrupted');
            A.areEqual(route.name, route2.name, 'name was corrupted');
