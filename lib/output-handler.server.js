...
OutputHandler.prototype = {
    ...
    flush: function(data, meta) {
        ...
        this.logger.log('done', 'info', 'server output adapter');
        this._readMeta(meta);
        this._writeHeaders();
        if (!data ||
        ...
        this.res.end(data);
        this.logger.log('END', 'mojito', 'server output adapter');
        ...
        if (this.memDebugObjects) {
            for (name in this.memDebugObjects) {
                if (this.memDebugObjects.hasOwnProperty(name)) {
                    ...
                    try {
                        size = this._stringify(obj, [name]).length;
                    } catch(err) {
            ...
            console.error('--MEMDEBUG-- ' + JSON.stringify(memDebug));
        }
    },
    ...
    error: function(err) {
        ...
        if (err.code === 404) {
            ...
            this.logger.log(err, 'warn', 'Server OutputHandler');
        } else {
            this.logger.log(err, 'error', 'Server OutputHandler');
        }
    ...
    _stringify: function(obj, name) {
        var str,
            i;

        // uncomment this to debug circular structures...
        // console.error('------------------------------- ' + name.join('.'));
        // ... then use techniques below to blacklist those members

        if ('YUI.process.mainModule.children' === name.join('.')) {
            return 'YUI.process.mainModule.children circular reference';
        }
        i = name.length - 1;
        if ('_handle'  === name[i]) { return name.join('.') + ' circular reference'; }
        if ('_handles' === name[i]) { return name.join('.') + ' circular reference'; }
        if ('_yuiaop'  === name[i]) { return name.join('.') + ' circular reference'; }
        if ('_yuievt'  === name[i]) { return name.join('.') + ' circular reference'; }
        if ('context'  === name[i]) { return name.join('.') + ' circular reference'; }
        if ('host'     === name[i]) { return name.join('.') + ' circular reference'; }

        try {
            str = JSON.stringify(obj);
        } catch(err) {
            if ('Converting circular structure to JSON' === err.message && 'object' === typeof obj) {
                str = '';
                for (i in obj) {
                    if (obj.hasOwnProperty(i) && 'object' === typeof obj[i]) {
                        str += this._stringify(obj[i], name.concat([i]));
                    }
                }
            } else {
                throw err;
