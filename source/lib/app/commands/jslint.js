...
function run(params, options) {
    ...
    if (!params || params.length === 0) {
        ...
        excludes = [
            ...
            /\/lib\/tests$/,
            /\/lib\/libs$/,
            /\/lib\/app\/autoload\/transport$/,
            /\/lib\/app\/autoload\/handlebars\.common\.js$/,
            /\/lib\/app\/libs$/,
            /\/lib\/management$/,
            /\/node_modules$/,
    ...
    process.stdout.write(errors + ' errors found.\n', 'utf8',
        function() {
            // TODO: The following is commented out until we figure out
            // how to flush all of our streams before the process exits.
            // Right now, output is lost if we exit like this.
            // if (errors) {
            //     process.exit(2);
            // }
        });
